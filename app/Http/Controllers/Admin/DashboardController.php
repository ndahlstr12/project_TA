...
class DashboardController extends Controller
{
    ...
    {
        // Data ringkas dari tabel DASHBOARD_LAPORAN
        $reportData = DB::table('DASHBOARD_LAPORAN')->first();
        ...
        $allTeachers = \App\Models\User::whereIn('role', ['guru', 'walikelas'])
            ->with('guru')
            ->get()
            ->map(function($user) {
                if ($user->role === 'walikelas') {
        ...
        $pdf = Pdf::loadView('admin.reports.dashboard_pdf', compact('stats', 'allTeachers'))
                  ->setPaper('a4', 'portrait');

        return $pdf->download('Laporan_Dashboard_Admin_' . date('Y-m-d') . '.pdf');
    }
}
