...
<div class="space-y-12">
    ...
    <div class="relative overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] bg-navy-950 p-6 md:p-12 text-white shadow-2xl shadow-navy-950/20">
        <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div class="max-w-xl text-center lg:text-left">
                <div class="flex items-center justify-center lg:justify-start gap-3 mb-4">
                    <span class="px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] border border-white/10">Sistem Manajemen Terpadu</span>
                </div>
                ...
                <p class="text-navy-100 text-xs md:text-base font-medium opacity-80 leading-relaxed">
                    Pantau efisiensi operasional dan perkembangan akademik sekolah secara real-time. Semua kontrol ada dalam genggaman Anda.
                </p>
            </div>
            <div class="shrink-0 flex justify-center lg:justify-end gap-4">
                <a href="{{ route('admin.dashboard.export-pdf') }}" class="group flex items-center gap-3 bg-white text-navy-950 px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-gold-400 transition-all duration-500 shadow-xl active:scale-95">
        ...
        <!-- Decorative Elements -->
        <div class="absolute top-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-gold-400/10 rounded-full blur-[60px] md:blur-[100px] -translate-y-1/2 translate-x-1/2"></div>
        <div class="absolute bottom-0 left-0 w-48 h-48 md:w-64 md:h-64 bg-blue-500/10 rounded-full blur-[50px] md:blur-[80px] translate-y-1/2 -translate-x-1/2"></div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        @php
            $stats_data = [
                ...
                ['Rata Nilai', $stats['rata_rata_nilai'], 'award', 'bg-purple-500', 'purple'],
                ['Presensi', $stats['kehadiran_rata'], 'check-square', 'bg-emerald-500', 'emerald'],
                ['Variabel', $stats['total_kriteria'], 'pie-chart', 'bg-orange-500', 'orange']
            ];
        @endphp
        ...
        <div class="group relative card-pro p-1 hover:border-{{ $stat[4] }}-500/50 transition-all duration-500 overflow-hidden">
            <div class="p-4 md:p-6">
                <div class="flex items-start justify-between">
                    <div class="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl {{ $stat[3] }} text-white flex items-center justify-center shadow-2xl shadow-{{ $stat[4] }}-500/20 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                        <i data-lucide="{{ $stat[2] }}" class="w-5 h-5 md:w-7 md:h-7"></i>
                    </div>
                    <div class="text-right">
                        <span class="text-[8px] md:text-[9px] font-black text-neutral-400 uppercase tracking-widest">{{ $stat[0] }}</span>
                        <h3 class="text-xl md:text-3xl font-black text-neutral-900 dark:text-white mt-1 tracking-tighter">{{ $stat[1] }}</h3>
                    </div>
                </div>
                <div class="mt-6 md:mt-8 flex items-center gap-2">
                    <div class="flex -space-x-1 md:-space-x-2">
                        @for($i=1; $i<=3; $i++)
                        <div class="w-5 h-5 md:w-6 md:h-6 rounded-full border-2 border-white dark:border-navy-900 bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-[7px] md:text-[8px] font-bold">
                            {{ $i }}
                        </div>
                        @endfor
                    </div>
                    <span class="text-[7px] md:text-[9px] font-bold text-neutral-400 uppercase tracking-tighter">Updated</span>
                </div>
            </div>
            <!-- Background Decoration -->
            <div class="absolute -right-8 -bottom-8 w-24 h-24 md:w-32 md:h-32 {{ $stat[3] }} opacity-[0.03] rounded-full group-hover:scale-150 transition-transform duration-700"></div>
        </div>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div class="lg:col-span-8 space-y-6 md:space-y-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between px-2 gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-neutral-100 dark:bg-white/5 flex items-center justify-center text-neutral-400 border border-base">
                        <i data-lucide="bar-chart-3" class="w-5 h-5 md:w-6 md:h-6"></i>
                    </div>
                    <div>
                        <h3 class="text-base md:text-lg font-black text-neutral-900 dark:text-white tracking-tight">Efikasi Kurikulum</h3>
                        <p class="text-[8px] md:text-[10px] text-neutral-400 font-bold uppercase tracking-widest mt-0.5">Analisis Sebaran Nilai Per-Kelas</p>
                    </div>
                </div>
                <div class="flex bg-neutral-100 dark:bg-white/5 p-1 rounded-xl w-fit">
                    <button class="px-4 py-2 text-[8px] md:text-[10px] font-black bg-white dark:bg-navy-800 text-neutral-900 dark:text-white rounded-lg shadow-sm uppercase tracking-widest">Ganjil</button>
                    <button class="px-4 py-2 text-[8px] md:text-[10px] font-black text-neutral-400 hover:text-neutral-900 uppercase tracking-widest transition-all">Genap</button>
                </div>
            </div>

            <div class="card-pro p-4 md:p-8 overflow-hidden">
                <div id="proChart" class="min-h-[300px] md:min-h-[350px]"></div>
            </div>

            <!-- Activity Stream -->
            <div class="space-y-4 md:space-y-6 pt-4">
                <div class="flex items-center justify-between px-2">
                    <h3 class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400">Log Aktivitas Terbaru</h3>
                    <button class="text-[9px] md:text-[10px] font-bold text-blue-500 uppercase tracking-widest hover:underline transition-all">Refresh</button>
                </div>

                <div class="md:card-pro md:overflow-hidden">
                    ...
                    <div class="md:hidden space-y-4">
                        @foreach($pendingTeachers as $teacher)
                        <div class="card-pro p-5 space-y-4 border-l-4 @if($teacher->status_tugas === 'Selesai') border-l-emerald-500 @else border-l-orange-500 @endif">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-black text-neutral-800 dark:text-neutral-200 truncate">{{ $teacher->name }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="px-2 py-0.5 bg-neutral-100 dark:bg-white/5 rounded text-[8px] font-black text-neutral-400 uppercase border border-base tracking-tighter">
                                            {{ $teacher->role }}
                                        </span>
                                        <span class="text-[8px] text-neutral-400 font-bold uppercase">ID: {{ $teacher->guru_id ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center justify-between gap-4 pt-3 border-t border-base">
                                <div class="flex items-center gap-2">
                                    <div class="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                                    <span class="text-[10px] font-bold text-neutral-600 dark:text-neutral-400 uppercase tracking-widest">{{ $teacher->tipe_tugas }}</span>
                                </div>
                                <div>
                                    @if($teacher->status_tugas === 'Selesai')
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-600 text-[8px] font-black uppercase tracking-widest">
                                        Tuntas
                                    </span>
                                    @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-500/10 text-orange-600 text-[8px] font-black uppercase tracking-widest animate-pulse">
                                        Pending
                                    </span>
                                    @endif
                                </div>
        ...
        <div class="lg:col-span-4 space-y-6 md:space-y-8">
            <div class="card-pro p-6 md:p-8 bg-neutral-900 text-white relative overflow-hidden">
